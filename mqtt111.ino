...
// CÓDIGO PARA nodeMCU de IP 111 conectada via protocolo MQTT
...
void callback(String topic, byte* payload, unsigned int length) {
  if(topic == PROTOCOLCODEST)
  {
    ...
    if(msg.equals("2")) /*SE RECEBER O CÓDIGO RELACIONADO A ESTA UNIDADE ELA RETORNA CÓDIGO DE STATUS OK*/
      client.publish(OKSTATUSMQTT, "33"); //'0b00100001'
    if(msg.equals("234"))/*SE RECEBER O CÓDIGO REFERENTE AO SENSOR ANALÓGICO*/
    {
      char analog[] = "";
      sprintf(analog,"%d", analogRead(analogInPin));
      client.publish(ANALT, analog); //'toRequest[0]' ANAL // /* ENVIA O VALOR DO POTENCIOMENTRO */
    }
    if(msg.equals("235")){  /*SER RECEBER O CÓDIGO REFERENTE AO SENSOR DIGITAL 1 um*/
    ...
    if(msg.equals("236")){  /*SER RECEBER O CÓDIGO REFERENTE AO SENSOR DIGITAL 2 dois*/
      char digital1[] = "";
      sprintf(digital1,"%d", digitalRead(buttonD1));
      client.publish(SD2GT, digital1);
    }
    if(msg.equals("237")){  /* SE RECEBER CÓDIGO PARA ATIVAR LED*/
      digitalWrite(led_pin, LOW);
      char led[] = "";
      sprintf(led,"%d", LOW);
      client.publish(LEDT, led);
    }
    if(msg.equals("238")){  /* SE RECEBER CÓDIGO PARA DESATIVAR LED*/
      digitalWrite(led_pin, HIGH);
      char led[] = "";
      sprintf(led,"%d", HIGH);
      client.publish(LEDT, led);
    }
    if(msg.equals("0")){ /*BROADCAST se receber o valor 0 (zero) responde com ID da node que nesse caso é 2. mas poderia ser qualquer outro valor */      
      client.publish(ALLBROADCASTT, "2");
    }    
    client.publish("INO109", "111 ip F    U   N   F   A   !!!\n");
  }
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    // Attempt to connect
    if (client.connect("ESP8266Client111JVSVL", MQTT_username, MQTT_password)) {
...
void loop() {
  ...
  client.loop();
//  if(!client.loop())
//    client.connect("ESP8266Client111JVSVL", MQTT_username, MQTT_password);  
}
